...
* `max_turns`를 최초 질문 이후 허용되는 추가 질문 슬롯 수로 정의하고 백엔드·프론트엔드에서 `0~3` 범위로 제한한다. 각 슬롯에는 답변 상태에 따라 꼬리질문 또는 새 기본 질문 하나가 들어간다.
...
- [x] **1-4B / P0-2 — 질문 유형, 자료 흐름, 난이도 및 꼬리질문 재조정**
  ...
  - 꼬리질문은 최초 질문의 핵심 주제와 난이도를 유지하며 기본 질문당 최대 1회 생성한다. 꼬리질문 답변 뒤 남은 추가 질문 슬롯은 새 기본 질문에 사용한다.
  ...
  - **추가(같은 브랜치 후속 커밋)**: 답변이 실질 내용 없이 모르겠다/기억나지 않는다/준비 못 했다는 뜻을 표현하면(정규식이 못 잡는 뉘앙스 표현 포함) LLM이 이를 재분류해 답변 불가로 처리하고, 이 경우 정상 꼬리질문 대신 같은 주제의 쉬운 재질문을 1회만 제공한다. 쉬운 재질문에마저 명시적으로 답변하지 못하면 그 즉시 반환하고(LLM 재호출 없음) 다음 정상 질문으로 넘어간다. 프론트 질문 횟수 표현을 "꼬리질문 N/M"에서 "기본 질문 1회 + 추가 질문 0~3회, 쉬운 재질문은 별도 집계"로 변경한다.

* [x] **1-4C / P0-3 — 발표 코칭 및 자료 수정 피드백 강화**
  ...
  * main 브랜치를 선병합해 답변별 상세 교정 UI(PR #15)와 슬라이드 커버리지 상용구 제외 개선을 함께 반영한다.

* [ ] **1-4D / P0-4 — 실제 비언어 정보 기반 피드백** -> fix-speech로 개선
  ...
  * 의존성: `useSpeechRecognition`에서 발화 시작/종료 타임스탬프 확보, `AudioContext`/`AnalyserNode` 기반 음량 샘플링 — 별도 조사 필요.
...
확인(재질문/뉘앙스 재분류/main 병합, 이번 PR):

- 대본만 입력했을 때, 슬라이드만 입력했을 때 각각 스파링이 시작되는지 확인한다.
- 기본 질문 1회는 항상 생성되고, 추가 질문 횟수를 0으로 설정하면 기본 질문 직후 다음 평가자로 넘어가는지 확인한다.
- 추가 질문 횟수를 1~3으로 설정하면 꼬리질문 또는 새 기본 질문으로 설정한 슬롯 수만큼 진행되는지 확인한다.
- 명시적 답변 불가("모르겠습니다" 등)와 뉘앙스형 답변 불가(예: "그건 준비를 못 해서 잘 모르겠는데요")가 모두 감지돼 채점 대신 힌트와 쉬운 재질문을 받는지 확인한다.
- 불확실 표현("정확하진 않지만")이 있어도 실질적인 답변 내용이 이어지면 정상 채점되는지 확인한다.
- 쉬운 재질문에마저 명시적으로 답변하지 못하면 반복 질문 없이 바로 다음 정상 질문으로 넘어가는지, 이때 LLM 평가 호출이 생략되는지 확인한다(뉘앙스형 재차 답변 불가는 재분류를 위해 LLM을 호출하되 결과는 동일하게 처리됨).
- 화면에 표시되는 남은 질문 횟수가 쉬운 재질문 여부와 무관하게 정상 질문 기준으로만 감소하는지 확인한다.
- 리포트에서 답변별 상세 교정(main), 슬라이드 커버리지(상용구 제외 개선 포함), 대본 수정 제안·답변 구조 코칭(1-4C)이 모두 표시되는지 확인한다.
- 기존 localStorage 세션(신규 필드 없는 과거 리포트)이 히스토리 화면에서 오류 없이 표시되는지 확인한다.
...
- [x] 3-3: 슬라이드별 구체적 누락 피드백 @jupitertreee
