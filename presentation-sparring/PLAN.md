...
- 작업:
  - `max_turns`를 최초 질문 이후 허용되는 최대 꼬리질문 횟수로 유지한다.
  - 백엔드와 프론트엔드에서 최대 꼬리질문 횟수를 `0~3`으로 제한한다.
  - 최초 질문 1회와 꼬리질문 최대 3회를 합쳐 persona당 최대 4회의 질의응답이 가능하도록 한다.
  - 설정 화면에서 `max_turns`가 전체 질문 수가 아닌 꼬리질문 횟수임을 명확히 안내한다.
  - 최초 질문은 발표 내용에서 가장 중요한 부족 지점 하나만 선택해 질문하도록 프롬프트를 정리한다.
  - 난이도가 높더라도 여러 약점을 한 질문에 동시에 묻지 않고, 한 번에 하나의 핵심 쟁점만 다루도록 한다.
  - 답변 평가는 질문에 대한 직접성, 구체적인 근거, 논리적 연결을 기준으로 수행한다.
  - 꼬리질문은 직전 질문과 직전 답변에서 확인된 부족한 지점을 이어서 묻도록 한다.
  - 이미 충분히 설명한 내용을 반복해서 묻거나 새로운 주제로 벗어나는 꼬리질문을 방지한다.
  - 답변이 충분한 경우 남은 횟수가 있어도 `followup`을 `null`로 반환한다.
  - mock provider에서도 꼬리질문 최대 3회의 흐름을 확인할 수 있도록 테스트 응답을 정리한다.
- 완료 기준:
  ...
  - 꼬리질문은 직전 답변에서 부족했던 구체적인 내용을 반영한다.
...
- 확인:
  ...
  - mock provider에서 최초 질문과 꼬리질문 최대 3회 흐름 확인
  - 실제 OpenAI provider에서 질문과 꼬리질문의 문맥 연결 확인
  - `npm run build` 및 브라우저 전체 스파링 흐름 확인
- 의존성:
  - Step 0-2
  - Step 1-1
  - Step 1-3
